  ...
  return (
    <div className="pt-24 min-h-screen bg-cyber-950 text-white relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 relative z-10">
        <Link to={PageRoute.HOME} className="inline-flex items-center gap-2 text-gray-400 hover:text-red-400 transition-colors mb-8 group">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-16">
          ...
          <div className="space-y-6">
            ...
            >
              ...
              <div className="flex gap-2 mb-6 bg-cyber-900 p-1 rounded-xl">
                {Object.keys(items).map(key => (
                  <button
                    key={key}
                    onClick={() => setActiveItem(key)}
                    className={`flex-1 py-2 text-xs font-bold rounded-lg capitalize transition-all ${activeItem === key ? 'bg-white/10 text-white' : 'text-gray-500 hover:text-gray-300'}`}
                  >
                    {key}
                  </button>
                ))}
